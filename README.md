...
Before that I spent a semester at Longhorn Life Sciences designing the Firestore schema and role-based access control behind a pre-launch clinical wound-monitoring platform, along with security rules and per-patient storage scoping so clinicians only reach their own records. This fall I'm joining PwC as a software engineering intern.

**A few things I've built**

- **[LedgerFlow](https://github.com/ygnewyork/ledgerflow)**: a real-time financial event platform, where I built a double-entry ledger Postgres keeps balanced, an outbox so crashes never double-charge a payment (12.9ms p99), and PySpark pipelines behind fraud scoring.
